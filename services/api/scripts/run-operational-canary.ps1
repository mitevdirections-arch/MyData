...
if ($PrintRoutes) {
  ...
  Write-Host "[operational-canary] startup route dump enabled (max=$RoutesMax)"
} else {
...
Write-Host "[operational-canary] starting on http://127.0.0.1:$Port"
py -m uvicorn app.main_operational_canary:app --reload --host 127.0.0.1 --port $Port --log-level debug
