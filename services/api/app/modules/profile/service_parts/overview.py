...
from app.modules.licensing.core_catalog import canonical_plan_code
...
class ProfileOverviewMixin:
    def superadmin_platform_overview(self, db: Session, *, actor: str) -> dict[str, Any]:
        ...
        active_leased_users = int(
            db.query(func.count(func.distinct(DeviceLease.user_id)))
            .filter(DeviceLease.is_active == True, DeviceLease.state == "ACTIVE")  # noqa: E712
            .scalar()
            or 0
        )
        ...
        for _, plan in db.query(License.tenant_id, License.module_code).filter(License.license_type == "CORE", License.status == "ACTIVE", License.valid_from <= now, License.valid_to >= now).all():
            code = canonical_plan_code(str(plan or "").strip().upper(), default="")
        ...
        top_usage = (
            db.query(DeviceLease.tenant_id, func.count(func.distinct(DeviceLease.user_id)).label("active_users"))
            .filter(DeviceLease.is_active == True, DeviceLease.state == "ACTIVE")  # noqa: E712
            .group_by(DeviceLease.tenant_id)
            .order_by(func.count(func.distinct(DeviceLease.user_id)).desc())
            .limit(10)
            .all()
        )
