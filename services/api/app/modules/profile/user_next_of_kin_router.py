...
def _resolve_scope_or_400(claims: dict[str, Any], workspace: str | None, db: Session) -> tuple[str, str]:
    try:
        ...
        detail = str(exc)
        code = 403 if detail in {
            "platform_workspace_requires_superadmin",
            "support_session_required_for_tenant_scope",
            "tenant_admin_required",
            "support_session_invalid_or_expired",
        } else 400
...
def create_user_next_of_kin(
    ...
    write_audit(
        db,
        action="profile.user.next_of_kin.upsert",
        actor=actor,
...
def delete_user_next_of_kin(
    ...
    write_audit(
        db,
        action="profile.user.next_of_kin.delete",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
