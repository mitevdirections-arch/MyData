...
class UsersMembershipMixin:
    ...
    def provision_workspace_user(
        ...
        *,
        ...
        user_payload = self._as_dict(data.get("user"))
        if not user_payload:
            user_payload = {
                key: data[key]
                for key in (
        ...
        user_out = self.upsert_workspace_user(
            db,
            workspace_type=workspace_type,
            workspace_id=workspace_id,
            user_id=user_id,
        ...
        role_codes_payload = data.get("role_codes")
        if role_codes_payload is not None:
            user_out = self.set_workspace_user_roles(
                db,
                workspace_type=workspace_type,
                workspace_id=workspace_id,
                user_id=user_id,
                role_codes=list(role_codes_payload or []),
                actor=actor,
            )

        credentials_cfg = self._as_dict(data.get("credentials"))
        raw_mode = (
            self._clean_text(data.get("credential_mode"), 32)
            or self._clean_text(credentials_cfg.get("mode"), 32)
            or "INVITE"
        )
        credential_mode = str(raw_mode).strip().upper().replace("-", "_")
        reset_existing = bool(
            credentials_cfg.get("reset_existing", data.get("reset_credentials_if_exists", False))
        )

        credentials_result = None
        if credential_mode in {"NONE", "SKIP"}:
            credentials_result = None
        elif credential_mode in {"INVITE", "INVITE_LINK"}:
            cred_payload = dict(credentials_cfg)
            cred_payload.pop("mode", None)
            cred_payload.pop("reset_existing", None)
            credentials_result = self.issue_user_invite(
                db,
                workspace_type=workspace_type,
                workspace_id=workspace_id,
                user_id=user_id,
        ...
        elif credential_mode in {"TEMP_PASSWORD", "PASSWORD_TEMP", "PASSWORD"}:
            cred_payload = dict(credentials_cfg)
            cred_payload.pop("mode", None)
            cred_payload.pop("reset_existing", None)
            credentials_result = self.issue_user_credentials(
                db,
                workspace_type=workspace_type,
                workspace_id=workspace_id,
                user_id=user_id,
            ...
            raise ValueError("credential_mode_invalid")

        return {
            "ok": True,
            "workspace_user": user_out,
