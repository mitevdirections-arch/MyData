...
def update_user_profile(
    ...
    write_audit(
        db,
        action="users.user.profile.update",
        actor=actor,
...
def create_user_contact(
    ...
    write_audit(
        db,
        action="users.user.contact.upsert",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def delete_user_contact(
    ...
    write_audit(
        db,
        action="users.user.contact.delete",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def create_user_address(
    ...
    write_audit(
        db,
        action="users.user.address.upsert",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def delete_user_address(
    ...
    write_audit(
        db,
        action="users.user.address.delete",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def create_user_document(
    ...
    write_audit(
        db,
        action="users.user.document.upsert",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def delete_user_document(
    ...
    write_audit(
        db,
        action="users.user.document.delete",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
def _credential_error_status(detail: str) -> int:
    if detail in {"tenant_not_found", "workspace_user_not_found", "user_membership_required", "credential_not_found"}:
        return 404
    if detail in {
...
def get_user_credentials(
    ...
    try:
        ...
        detail = str(exc)
        raise HTTPException(status_code=_credential_error_status(detail), detail=detail) from exc
    return {"ok": True, "item": out}
...
def issue_user_credentials(
    ...
    write_audit(
        db,
        action="users.user.credentials.issue",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
...
@router.post("/admin/users/{user_id}/credentials/invite")
def issue_user_invite(
    ...
    actor = str(claims.get("sub") or "unknown")
    try:
        out = user_domain_service.issue_user_invite(
            db,
            workspace_type=wtype,
            workspace_id=wid,
            user_id=user_id,
            actor=actor,
            payload=payload,
            reset_existing=bool((payload or {}).get("reset_existing", False)),
        )
    except ValueError as exc:
        detail = str(exc)
        raise HTTPException(status_code=_credential_error_status(detail), detail=detail) from exc
    ...
    write_audit(
        db,
        action="users.user.credentials.invite",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
        target=f"user-credentials/{wtype}/{wid}/{user_id}",
        metadata={"workspace_type": wtype, "workspace_id": wid},
...
def reset_user_password(
    ...
    write_audit(
        db,
        action="users.user.credentials.reset",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
        target=f"user-credentials/{wtype}/{wid}/{user_id}",
        metadata={"workspace_type": wtype, "workspace_id": wid},
...
@router.post("/admin/users/{user_id}/credentials/lock")
def lock_user_credential(
    ...
    actor = str(claims.get("sub") or "unknown")
    try:
        out = user_domain_service.lock_user_credential(
            db,
            workspace_type=wtype,
            workspace_id=wid,
            user_id=user_id,
    ...
    write_audit(
        db,
        action="users.user.credentials.lock",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
        target=f"user-credentials/{wtype}/{wid}/{user_id}",
        metadata={"workspace_type": wtype, "workspace_id": wid},
...
@router.post("/admin/users/{user_id}/credentials/unlock")
def unlock_user_credential(
    ...
    actor = str(claims.get("sub") or "unknown")
    try:
        out = user_domain_service.unlock_user_credential(
            db,
            workspace_type=wtype,
            workspace_id=wid,
            user_id=user_id,
    ...
    write_audit(
        db,
        action="users.user.credentials.unlock",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
        target=f"user-credentials/{wtype}/{wid}/{user_id}",
        metadata={"workspace_type": wtype, "workspace_id": wid},
...
@router.post("/admin/users/{user_id}/credentials/revoke-invite")
def revoke_user_invite(
    ...
    actor = str(claims.get("sub") or "unknown")
    try:
        out = user_domain_service.revoke_user_invite(
            db,
            workspace_type=wtype,
            workspace_id=wid,
            user_id=user_id,
    ...
    write_audit(
        db,
        action="users.user.credentials.revoke_invite",
        actor=actor,
        tenant_id=_tenant_for_audit(wtype, wid),
        target=f"user-credentials/{wtype}/{wid}/{user_id}",
        metadata={"workspace_type": wtype, "workspace_id": wid},
